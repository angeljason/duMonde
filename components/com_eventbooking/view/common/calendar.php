					...
					<li class="eb-calendarDay calendar-day regpro-calendarDay <?php echo $rowClass; ?>">
						...
						foreach ($currentDay["events"] as $key=> $event)
						{
							...
							<div class="date day_cell">
								<a class="eb_event_link" href="<?php echo JRoute::_(EventbookingHelperRoute::getEventRoute($event->id, isset($categoryId) ? $categoryId : 0, $Itemid)); ?>" title="<?php echo $event->title; ?>" <?php if ($color) echo 'style="background-color:#'.$color.'";' ; ?> target="_blank">
