...
class os_paypal extends os_payment {
	...
	function processPayment($row, $data) {
		...
		$this->setParam('cancel_return', $siteUrl.'index.php?option=com_pmform&view=cancel&id='.$row->id.'&Itemid='.$Itemid);
		$this->setParam('notify_url', $siteUrl.'index.php?option=com_pmform&task=payment_confirm&payment_method=os_paypal');		
		$this->setParam('address1', $row->address);		
		$this->setParam('address2', $row->address2);		
		$this->setParam('city', $row->city);
        if ($row->country)
        {
            $this->setParam('country', PMFormHelper::getCountryCode($row->country));
        }
        else
        {
            $this->setParam('country', PMFormHelper::getCountryCode($config->default_country));
        }
	...
	function submitPost() {
	?>
		<div class="contentheading"><?php echo  JText::_('PF_WAIT_PAYPAL'); ?></div>
