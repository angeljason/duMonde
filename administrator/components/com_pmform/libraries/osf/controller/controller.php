...
class POSFController
{
	 ...
	 * @param array $config        	
	 *
	 * @return POSFController
	 */
	public static function getInstance($option, POSFInput $input = null, array $config = array())
	{
		...
		$input = $input ? $input : new POSFInput();
		...
		{
			$name = POSFInflector::singularize($input->get('view'));
			if (!$name)
			{
				$name = 'controller';
			}
		...
		if (!isset(self::$instances[$component . $name]))
		{
			...
			if ($view)
			{
				$class = $prefix . 'Controller' . ucfirst(POSFInflector::singularize($view));
			}
			else
			{
	...
	public function __construct(POSFInput $input = null, array $config = array())
	{
		...
		$xMethods = get_class_methods('POSFController');
	 ...
	 *
	 * @return POSFController return itself to support changing
	 */
	 ...
	 *        		        	
	 * @return POSFController A POSFController object to support chaining.
	 */
	...
	public function getModel($name = '', array $config = array())
	{
		...
		{
			if (POSFInflector::isPlural($name))
			{
				$config['default_model_class'] = 'POSFModelList';
			}
			else
			{
				if ($this->app->isAdmin())
				{
					$config['default_model_class'] = 'POSFModelAdmin';
				}
				else
				{
					$config['default_model_class'] = 'POSFModelItem';
				}
			}
		}
		
		//Create model and auto populate model states if required
		$model = POSFModel::getInstance($name, $this->classPrefix . 'Model', $config);
	 ...
	 *        	
	 * @return POSFView Reference to the view
	 *        
	 */
	public function getView($name, $type = 'html', $layout = 'default', array $config = array())
	{
		...
		if (!isset($config['default_view_class']))
		{
			if (POSFInflector::isPlural($name))
			{
				$config['default_view_class'] = 'POSFViewList';
			}
			else
			{
				$config['default_view_class'] = 'POSFViewItem';
			}
		}
		if ($this->app->isAdmin())
		{
		...
		return POSFView::getInstance($name, $type, $this->classPrefix . 'View', $config);
	}

	/**
	 * Populate model states from controller input
	 * 
	 * @param POSFModel $model
	 */
	 ...
	 *        	
	 * @return POSFController This object to support chaining.
	 *        
	 */
	 ...
	 *
	 * @return POSFInput The input object.
	 *        
	 */
