...
class plgHikashopshippingCANADAPOST extends hikashopShippingPlugin {
	...
	function _RequestMethods($data, $xml) {
		...
		foreach($xml->ratesAndServicesResponse->product as $rate){
			$shipment[$i]['value'] = $rate->rate;
			$shipment[$i]['name'] = $rate->name;
			$shipment[$i]['shippingDate'] = $rate->shippingDate;
			$shipment[$i]['deliveryDate'] = $rate->deliveryDate;
			$shipment[$i]['deliveryDayOfWeek'] = $rate->deliveryDayOfWeek;
			$shipment[$i]['nextDayAM'] = $rate->nextDayAM;
			$shipment[$i]['status_code'] = $xml->ratesAndServicesResponse->statusCode;
			$shipment[$i]['status_message'] = $xml->ratesAndServicesResponse->statusMessage;
			$i++;
		}
		return $shipment;
	}
