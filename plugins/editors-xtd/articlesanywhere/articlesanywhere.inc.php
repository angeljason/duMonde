		...
		if ($k2 && $content_type == 'k2') {
			...
			if ($filter_search) {
				if ($params->adminSearch == 'full') {
					$where[] = 'MATCH(c.title, c.introtext, c.`fulltext`, c.extra_fields_search, c.image_caption,c.image_credits,c.video_caption,c.video_credits,c.metadesc,c.metakey) AGAINST(' . $db->q($filter_search) . ')';
				} else {
					$where[] = 'MATCH( c.title ) AGAINST(' . $db->q($filter_search) . ')';
				}
			}
			...
			$query->select('COUNT(*)');
			$query->from('#__k2_items AS c');
			$query->join('LEFT', '#__k2_categories AS cc ON cc.id = c.catid');
			$query->where($where);
			$db->setQuery($query);
			$total = $db->loadResult();
			...
			$query = $db->getQuery(true);
			$query->select('c.*, g.title AS accesslevel, cc.name AS category, v.name AS author');
			$query->select('w.name AS moderator, u.name AS editor');
			$query->from('#__k2_items AS c');
			$query->join('LEFT', '#__k2_categories AS cc ON cc.id = c.catid');
			...
			$query->join('LEFT', '#__users AS w ON w.id = c.modified_by');
			$query->where($where);
			$query->order($order);
			...
			// Keyword filter
			if ($filter_search) {
				$where[] = '(LOWER( c.title ) LIKE ' . $db->q('%' . $db->escape($filter_search, true) . '%', false)
					. ' OR c.id = ' . (int) $filter_search . ' )';
			}
				...
				<?php if ($k2) : ?>
					<table class="table table-striped">
						<tr>
							...
							<td>
								<label rel="tooltip" title="<?php echo JText::_('AA_CONTENT_TYPE_CORE_DESC'); ?>">
									<input onchange="form.submit()" type="radio" name="content_type" id="content_type_core"
										value="core" <?php echo ($content_type == 'core') ? 'checked="checked"' : ''; ?>>
									<?php echo JText::_('AA_CONTENT_TYPE_CORE'); ?>
								</label>
							</td>
							<td>
								<label rel="tooltip" title="<?php echo JText::_('AA_CONTENT_TYPE_K2_DESC'); ?>">
									<input onchange="form.submit()" type="radio" name="content_type" id="content_type_k2"
										value="k2" <?php echo ($content_type == 'k2') ? 'checked="checked"' : ''; ?>>
			...
			<tbody>
				...
				foreach ($rows as $row) {
					...
					<tr class="<?php echo "row$k"; ?>">
						<td class="center">
							<?php echo '<label rel="tooltip" title="<strong>' . JText::_('AA_USE_ID_IN_TAG') . '</strong><br/>{article k2:' . $row->id . '}...{/article}"><a href="javascript://" onclick="articlesanywhere_jInsertEditorText( \'' . $row->id . '\' )">' . $row->id . '</a></label>'; ?>
						</td>
						<td class="center">
						...
						<td class="title">
							<?php echo '<label rel="tooltip" title="<strong>' . JText::_('AA_USE_ALIAS_IN_TAG') . '</strong><br/>{article k2:' . $row->alias . '}...{/article}"><a href="javascript://" onclick="articlesanywhere_jInsertEditorText( \'' . $row->alias . '\' )">' . $row->alias . '</a></label>'; ?>
						</td>
						<td>
