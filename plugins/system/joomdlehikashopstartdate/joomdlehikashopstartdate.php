...
class  plgSystemJoomdlehikashopstartdate extends JPlugin
{
			...
			foreach ($products as $product)
			{
				...
				if (in_array ($courses_category, $cats))
				{
					...
					else 
					{
						...
						{
							//course
							// Check if course has enrol start date param
							$enrol_date = '';
							$params = unserialize ($product->order_product_options);
							foreach ($params as $param) {

								$enrol_date = $param->characteristic_value;
								break; // This assumes enrol date will be the first attribute in product
							}
							if ($enrol_date)
							{
								$this->enrol_user_with_start_date ($user->username, $product->order_product_code, $enrol_date);
							}
							else
