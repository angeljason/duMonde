...
class plgSystemArticlesAnywhereHelper
{
	function __construct(&$params)
	{
		...
		if (NNFrameworkFunctions::extensionInstalled('k2')) {
			$selects = $db->getTableColumns('#__k2_items');
			if (is_array($selects)) {
				unset($selects['introtext']);
				unset($selects['fulltext']);
				$selects = array_keys($selects);
		...
		if (preg_match($regex_close, $string)) {
			...
			while ($count++ < 10 && preg_match($regex_close, $string) && preg_match_all($regex, $string, $matches, PREG_SET_ORDER) > 0) {
				foreach ($matches as $match) {
					if ($match['3'] == $this->params->articles_tag) {
						// Check for categories...
						$items = str_replace('|', ',[|],', $match['4']);
						$items = explode(',', $items);
						$ids = array();
						$i = 0;
						$iscat = 0;
						foreach ($items as $item) {
							...
							if (strpos($item, 'cat:') === 0 || strpos($item, 'cats:') === 0) {
								$i++;
								$iscat = 1;
								$ids[$i] = array(
									'ids' => array(),
								...
								$item = substr($item, strlen('cat:'));
							}

							if ($iscat) {
								if (preg_match('#^((?:[^:]+\:)+)([^:]+)$#', $item, $params)) {
									$item = $params['2'];
									$params = explode(':', trim(trim($params['1']), ':'));
									foreach ($params as $p) {
										if (is_numeric($p)) {
											$ids[$i]['limit'] = (int) $p;
										} else if (preg_match('#^([0-9]+)-([0-9]+)$#', $p, $limit)) {
											$ids[$i]['limit'] = (int) ($limit['2'] - $limit['1']) + 1;
											$ids[$i]['offset'] = (int) $limit['1'] - 1;
										} else {
											$ids[$i]['ordering'] = trim($p);
										}
									}
								}
								$ids[$i]['ids'][] = $item;
						...
						foreach ($items as $item) {
							if (is_array($item)) {
								$artids = $this->getIdsByCategories($item['ids'], $item['ordering'], $item['limit'], $item['offset']);
						...
						foreach ($ids as $id) {
							$m = $match;
							$m['4'] = trim($id);
							$html[] = $this->processMatch($string, $art, $m, $ignores, $message);
						}
						$string = str_replace($match['0'], implode('', $html), $string);
					} else {
	...
	function processArticle($id, $art, $text = '', $type = 'article', $ignores = array(), $first = 0)
	{
		...
		if (in_array($id, array('current', 'self', '{id}', '{title}', '{alias}'))) {
			if (isset($art->id)) {
				...
				$id = $match['1'];
			} else if (JFactory::getApplication()->input->get('option') == 'com_content' && JFactory::getApplication()->input->get('view') == 'article') {
		...
		if ($type == 'article') {
			...
			$query->select('c.parent_id AS parent');
		...
		foreach ($matches as $match) {
						...
						if (is_array($menuitems)) {
							foreach ($menuitems as $item) {
								if (
									...
									&& $item->query['view'] == 'item'
									&& isset($item->query['layout'])
									&& $item->query['layout'] == 'item'
									&& isset($item->query['id'])
							...
							if (!$id) {
								foreach ($menuitems as $item) {
									if (
										isset($item->query['view'])
										&& $item->query['view'] == 'itemlist'
										&& isset($item->query['layout'])
										&& $item->query['layout'] == 'category'
										&& isset($item->query['id'])
			...
			} else if (in_array($tag, array('image-intro', 'image-fulltext')) && isset($article->$tag)) {
				if ($tag == 'image-intro') {
	...
	function getExtraFieldValue(&$extra, $data, $catid)
	{
		...
		$query = $db->getQuery(true);
		$query->select('c.extraFieldsGroup');
		...
		$query = $db->getQuery(true);
		$query->select('e.*');
		$query->from('#__k2_extra_fields as e');
		$query->where('e.group = ' . (int) $extragroup);
		$query->where('e.published = 1');
		if (substr($data, 0, 6) == 'extra-' && is_numeric(substr($data, 6))) {
			$query->where('( e.id = ' . (int) substr($data, 6) . ' OR e.name = ' . $db->q($data) . ')');
		} else {
			$query->where('e.name = ' . $db->q($data));
		}
	...
	function getIdsByCategories($ids, $ordering = 'created DESC', $limit = 0, $offset = 0)
	{
		if (empty($ids)) {
			return array();
		}
		...
		if (!empty($wids)) {
			...
			$query->where('c.extension = ' . $db->q('com_content'));
			$query->where('(c.title IN (' . implode(',', $wids) . ') OR c.alias IN (' . implode(',', $wids) . '))');
			$db->setQuery($query);
			$dbids = $db->loadColumn();
			foreach ($dbids as $id) {
				$ids[] = $id;
		...
		$ordering = $ordering ? $ordering : $this->params->ordering;
		$limit = $limit ? $limit : ((int) $this->params->limit ? (int) $this->params->limit : 1000);
